    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW:
            if event.to_me:
                ...
                print('[INFO] Get msg from user [%d]: [%s], language: [%s]' % (user_id, msg, language))

                if keyboard is None and is_first_time_use(user_id):
                    sender(vk_session, 'user_id', user_id,
                           message=MSG_WELCOME,
                           keyboard=create_keyboard(CHANGE_LANGUAGE[Language.RUSSIA.value].lower()))
                    ...
                    if msg in SET_LANGUAGE:
                        ...
                        print('[INFO] Set user [%d] to [%s]' % (user_id, language))
                        continue
...
if __name__ == '__main__':
    try:
        ...
        print('[ERROR] Exception on line:[%s]' % traceback.print_exc())
        print('[WARNING] Ready to restart >>>>>')
        i = RESTART_WAIT_TIME
        while i > 0:
            print(">>> %d %s" % (i, ('.' * i)))
            sleep(1)
